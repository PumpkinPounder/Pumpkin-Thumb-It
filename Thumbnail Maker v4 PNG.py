...
# PNG save settings (new)
PNG_COMPRESS_LEVEL = 6  # 0 = fastest / largest, 9 = slowest / smallest
PNG_OPTIMIZE = True
...
# NOTE: function name kept for compatibility, but output is now PNG (screen.png)
def create_single_frame_jpg(video_path, cfg):
    try:
        ...
        img.save(out_path, format="PNG", compress_level=int(PNG_COMPRESS_LEVEL), optimize=bool(PNG_OPTIMIZE))
        return out_path
    except Exception:
        return None
...
def generate_thumbnail_sheet(video_path, cfg, anim_index=None):
    ...
    # CHANGED: output sheet is now PNG
    output_name = f"bitporn.eu_{base}.png"
    png_path = os.path.join(scr_dir, output_name)
    sheet.save(png_path, format="PNG", compress_level=int(PNG_COMPRESS_LEVEL), optimize=bool(PNG_OPTIMIZE))
