...
# ============================================================
# SHEET GENERATION (JPG + optional centerX.webp)
...
def generate_thumbnail_sheet(video_path, cfg, anim_index=None):
    ...
    output_name = f"sheet_{base}.jpg"
    jpg_path = os.path.join(scr_dir, output_name)

    # Save JPG (convert to RGB)
    sheet.convert("RGB").save(jpg_path, quality=cfg["JPG_QUALITY"], optimize=cfg["JPG_OPTIMIZE"], subsampling=2)
...
class ThumbnailMakerApp:
    ...
    def _build_ui(self):
        ...
        ttk.Label(header, text="Outputs JPG sheets and center1..5 WEBP into /scr next to each video.", style="Sub.TLabel").pack(anchor="w", pady=(2, 0))
