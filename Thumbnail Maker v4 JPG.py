...
LOGO_URL = "https://imghost.dev/images/2025/08/15/a964cace8868.png"
LOGO_MAX_W_PX = 350
LOGO_MAX_H_PX = 150
...
FOOTER_BG = (20, 20, 20)
FOOTER_TEXT = "Pumpkinpounder Bitporn.eu"
...
CENTERLONGEST_SECONDS = 8.0
CENTERLONGEST_FPS = 15
...
def _resolve_ff_tools():
    """
    Prefer configured paths; otherwise try PATH.
    Raises RuntimeError if tools cannot be found.
    """
...
def get_video_info(video_path):
    ...
    try:
        ...
        return {"width": 0, "height": 0, "fps": 0.0, "duration": 0.0, "size": 0, "codec": "?", "bitrate_kbps": 0}
    ...
    fps = 0.0
    afr = stream.get("avg_frame_rate") or "0/1"
    try:
        num, den = afr.split("/")
    ...
    br = stream.get("bit_rate") or fmt.get("bit_rate") or 0
    try:
        br = int(br)
    except Exception:
        br = 0
    bitrate_kbps = int(round(br / 1000.0)) if br else 0
...
def _cv2_resize_letterbox(frame_bgr, target_wh):
    tw, th = target_wh
...
def _pil_from_bgr(img_bgr):
    rgb = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2RGB)
    return Image.fromarray(rgb)


def _add_rounded_corners_pil(pil_img, radius=14):
...
def _build_layout_slots():
    ...
    x_big_left = SPACING
    y_top = y
    slots.append({"x": x_big_left, "y": y_top, "w": BIG_W, "h": BIG_H, "is_big": True})

    x_right_block = x_big_left + BIG_W + SPACING
    for r in range(2):
        for c in range(2):
            x = x_right_block + c * (SMALL_W + SPACING)
            yy = y_top + r * (SMALL_H + SPACING)
            slots.append({"x": x, "y": yy, "w": SMALL_W, "h": SMALL_H, "is_big": False})

    y = y_top + BIG_H + SPACING
    ...
    x_mid_left = SPACING
    slots.append({"x": x_mid_left, "y": y, "w": BIG_W, "h": BIG_H, "is_big": True})
    x_mid_right = x_mid_left + BIG_W + SPACING
    slots.append({"x": x_mid_right, "y": y, "w": BIG_W, "h": BIG_H, "is_big": True})
...
def draw_banner(sheet, draw, video_path, info, sheet_width):
    draw.rectangle([0, 0, sheet_width, BANNER_HEIGHT], fill=(0, 0, 0))
    ...
    lines = [
        ...
        f"Duration: 00:{format_time(dur)}",
        f"Video: {codec} (Main) :: {br} kb/s, {fps:.2f} fps",
        "Audio: AAC, 2 ch"
    ]
    ...
    right_pad = 16
    logo_gap = 18
    reserved = (LOGO_IMAGE.width + logo_gap) if LOGO_IMAGE else 0
    max_text_w = max(50, sheet_width - x - right_pad - reserved)

    for ln in lines:
        ln2 = _fit_text(draw, ln, font, max_text_w)
        draw.text((x, y), ln2, fill=(235, 235, 235), font=font)
        y += 22

    if LOGO_IMAGE:
        lx = sheet_width - LOGO_IMAGE.width - 18
        ly = 10
        sheet.paste(LOGO_IMAGE, (lx, ly), LOGO_IMAGE)
...
def draw_footer(sheet, sheet_width, sheet_height):
    ...
    draw.rectangle([0, y0, sheet_width, sheet_height], fill=FOOTER_BG)
...
def _timestamps_from_duration(duration_sec, count):
    try:
...
def _frame_targets_from_timestamps(timestamps_sec, fps):
    try:
...
def _make_placeholder(w, h, text="NO FRAME"):
    ...
    tw = bbox[2] - bbox[0]
    th = bbox[3] - bbox[1]
    d.text(((w - tw) // 2, (h - th) // 2), text, fill=(200, 200, 200), font=font)
    return img
...
def _ffmpeg_extract_frame_pil_scaled(video_path: str, time_sec: float, out_w: int, out_h: int):
    """
    Extract a single frame using ffmpeg at time_sec, scaled+letterboxed to out_w/out_h.
    """
    vf = (
...
def _ffmpeg_extract_frame_pil_raw(video_path: str, time_sec: float):
    """
    Extract a single frame using ffmpeg at time_sec, no scaling (keeps original).
    """
    cmds = [
        [
            FFMPEG, "-hide_banner", "-loglevel", "error",
...
def _extract_thumbs_with_slots(video_path, info, slots, cfg):

    timestamps_sec = _timestamps_from_duration(info.get("duration", 0.0), len(slots))
    if not timestamps_sec:
        return []

    force_ffmpeg = _is_wmv(video_path) or (str(info.get("codec", "")).lower() in ("vc1", "vc-1"))

    if force_ffmpeg:
        thumbs = []
        for idx, tsec in enumerate(timestamps_sec):
            slot = slots[idx]
            img = _ffmpeg_extract_frame_pil_scaled(video_path, tsec, slot["w"], slot["h"])
            if img is None:
                img = _make_placeholder(slot["w"], slot["h"])
            if not cfg["FAST_MODE"]:
                img = _add_rounded_corners_pil(img)
            thumbs.append(img)
        return thumbs
    ...
    if (not ok_open) or (not ok_first) or (frame0 is None):
        try:
            cap.release()
        except Exception:
            pass
    ...
    fps = info.get("fps", 0.0)
    frame_targets = _frame_targets_from_timestamps(timestamps_sec, fps)
    ...
    if not frame_targets:
        ...
        for idx, tsec in enumerate(timestamps_sec):
            ...
            if not ok or fr is None:
                ...
                thumb = _cv2_resize_letterbox(fr, (slot["w"], slot["h"]))
                img = _pil_from_bgr(thumb)
            if not cfg["FAST_MODE"]:
                img = _add_rounded_corners_pil(img)
            thumbs.append(img)
        cap.release()
    ...
    for idx, target in enumerate(frame_targets):
        if target > current_index + cfg["SEEK_GRAB_THRESHOLD"]:
            ...
            steps = max(0, target - current_index)
            ok2 = True
        ...
        slot = slots[idx]
        if frame is None:
            pil_img = _make_placeholder(slot["w"], slot["h"])
        else:
            thumb = _cv2_resize_letterbox(frame, (slot["w"], slot["h"]))
            pil_img = _pil_from_bgr(thumb)

        if not cfg["FAST_MODE"]:
            pil_img = _add_rounded_corners_pil(pil_img)
...
def _paste_thumb_in_card(sheet, thumb_pil, slot):
    x, y, w, h = slot["x"], slot["y"], slot["w"], slot["h"]
    left = x - CARD_BORDER
    top = y - CARD_BORDER
    right = x + w + CARD_BORDER
    bottom = y + h + CARD_BORDER

    draw = ImageDraw.Draw(sheet)
    draw.rectangle([left, top, right, bottom], fill=CARD_BG)
    sheet.paste(thumb_pil, (x + CARD_INNER_PAD, y + CARD_INNER_PAD))



def create_animated_sheet_webp(video_path, sheet_base_pil, slots, cfg, anim_index):
    ...
    try:
        ...
        seg_count = max(1, int(ANIMATED_SHEETS_PER_FOLDER))
        seg_len = max(0.001, duration_sec / seg_count)

        idx0 = max(0, min(seg_count - 1, int(anim_index) - 1))
        seg_start = idx0 * seg_len
        seg_end = min(duration_sec, seg_start + seg_len)
        seg_span = max(0.001, seg_end - seg_start)


        half = seg_span / 2.0
        left_start, left_end = seg_start, seg_start + half
        right_start, right_end = seg_start + half, seg_end

        left_span = max(0.001, left_end - left_start)
        right_span = max(0.001, right_end - right_start)


        clip_seconds = min(clip_req, left_span * 0.95, right_span * 0.95)
        clip_seconds = max(1.0, clip_seconds)

        num_frames = max(8, int(round(clip_seconds * webp_fps)))


        big_indices = [i for i, s in enumerate(slots) if s.get("is_big")]
        if len(big_indices) < 3:
            return None
        slot_a = slots[big_indices[-2]]
        slot_b = slots[big_indices[-1]]


        def build_ts_list(win_start, win_end, n):
            end_ts = min(win_end, win_start + clip_seconds)
            if end_ts <= win_start:
                end_ts = min(win_end, win_start + 1.0)
            if end_ts <= win_start:
                return []
            step = (end_ts - win_start) / max(1, (n - 1))
            return [win_start + i * step for i in range(n)]

        ts_left = build_ts_list(left_start, left_end, num_frames)
        ts_right = build_ts_list(right_start, right_end, num_frames)
        if not ts_left or not ts_right:
            return None

        scr_dir = os.path.join(os.path.dirname(video_path), "scr")
        os.makedirs(scr_dir, exist_ok=True)
        out_path = os.path.join(scr_dir, f"{ANIM_NAME_PREFIX}{anim_index}.webp")

        frames = []
        for i in range(min(len(ts_left), len(ts_right))):
            tA = ts_left[i]
            tB = ts_right[i]


            frame_sheet = sheet_base_pil.copy()


            imgA = _ffmpeg_extract_frame_pil_scaled(video_path, tA, slot_a["w"], slot_a["h"])
            if imgA is None:
                imgA = _make_placeholder(slot_a["w"], slot_a["h"])
            if not cfg["FAST_MODE"]:
                imgA = _add_rounded_corners_pil(imgA)
            frame_sheet.paste(imgA, (slot_a["x"] + CARD_INNER_PAD, slot_a["y"] + CARD_INNER_PAD))


            imgB = _ffmpeg_extract_frame_pil_scaled(video_path, tB, slot_b["w"], slot_b["h"])
            if imgB is None:
                imgB = _make_placeholder(slot_b["w"], slot_b["h"])
            if not cfg["FAST_MODE"]:
                imgB = _add_rounded_corners_pil(imgB)
            frame_sheet.paste(imgB, (slot_b["x"] + CARD_INNER_PAD, slot_b["y"] + CARD_INNER_PAD))
...
def create_single_frame_jpg(video_path, cfg):
    try:
        info = get_video_info(video_path)
        duration = float(info.get("duration") or 0.0) or 4.0
        ...
        scr_dir = os.path.join(os.path.dirname(video_path), "scr")
        os.makedirs(scr_dir, exist_ok=True)
        out_path = os.path.join(scr_dir, "screen.jpg")

        img.save(out_path, quality=cfg["JPG_QUALITY"], optimize=cfg["JPG_OPTIMIZE"], subsampling=2)
        return out_path
    except Exception:
        return None
...
def generate_thumbnail_sheet(video_path, cfg, anim_index=None):
    ...
    thumbs = _extract_thumbs_with_slots(video_path, info, slots, cfg)
    if not thumbs:
        return None, None


    if len(thumbs) < len(slots):
        for i in range(len(thumbs), len(slots)):
            s = slots[i]
            thumbs.append(_make_placeholder(s["w"], s["h"]))

    for i, thumb in enumerate(thumbs[:len(slots)]):
        _paste_thumb_in_card(sheet, thumb, slots[i])
    ...
    output_name = f"bitporn.eu_{base}.jpg"
    jpg_path = os.path.join(scr_dir, output_name)
    sheet.save(jpg_path, quality=cfg["JPG_QUALITY"], optimize=cfg["JPG_OPTIMIZE"], subsampling=2)
...
class ThumbnailMakerApp:
    def __init__(self):
        ...
        self.root.title("Thumbnail Maker v4 (Fixed WMV)")
    ...
    def _build_ui(self):
        ...
        ttk.Label(header, text="Thumbnail Maker v4 (WMV Fixed)", style="Title.TLabel").pack(anchor="w")
        ttk.Label(header, text="MADE FOR BITPORN.EU", style="Sub.TLabel").pack(anchor="w", pady=(2, 0))
        ...
        ttk.Label(
            footer,
            text="Made By Pumpkinpounder BitPorn - Porn With Purpose",
            style="Sub.TLabel"
        ).pack(side="right")
            ...
            for folder, vids in folder_map.items():
                ...
                if not self.state.stop_processing:
                    ...
                    if longest_in_folder:
                        ...
                        out2 = create_single_frame_jpg(longest_in_folder, cfg)
                        if out2:
                            self.log(f"🔸 screen.jpg saved: {out2}")
    ...
    def _process_one(self, video_path, cfg, anim_index):
        ...
        base = os.path.basename(video_path)
        jpg_path, webp_path = generate_thumbnail_sheet(video_path, cfg, anim_index=anim_index)
        if not jpg_path:
            return False, f"❌ Failed: {base} (no frames)"

        msg = f"✅ Finished: {base}  → {jpg_path}"
        if webp_path:
